...
export async function sendOTP(email: string): Promise<{ success: boolean; error?: string }> {
  ...
  if (!RESEND_API_KEY) {
    ...
    // In production without API key, still store OTP and return success
    // This allows the OTP to be verified via DB even if email wasn't sent
    console.warn("[AUTH] RESEND_API_KEY not set — OTP stored in DB but email not sent to: " + email);
    return { success: true };
  }

  // Use custom from address if configured, otherwise fall back to Resend sandbox
  // NOTE: onboarding@resend.dev only sends to the account owner's email on free tier.
  // For production, set RESEND_FROM_EMAIL to a verified custom domain address.
  const fromEmail = process.env.RESEND_FROM_EMAIL || "NotiFetch <onboarding@resend.dev>";
...
export const authOptions: NextAuthOptions = {
  ...
  // v2.9.97: Verbose logger so Vercel runtime logs show the *actual* OAuth error
  // instead of the generic "Callback" that NextAuth surfaces to the browser.
  // Check Vercel → Deployments → Functions → /api/auth/* logs after a failed login.
  logger: {
    error(code, ...message) {
  ...
  // v2.9.97: Explicit cookie config to work around __Host- prefix issues some
  // browsers exhibit when the OAuth redirect bounces through Google. Using
  // __Secure- prefix (less strict than __Host-) so cookies survive subdomain
  // redirects and are sent on the callback from Google.
  cookies: {
    csrfToken: {
