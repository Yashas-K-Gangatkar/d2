...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  // OAuth linking
  OAuthAccountNotLinked: "This email is already associated with another sign-in method. Sign in using the original method.",
  // OAuth flow errors
  ...
  EmailCreateAccount: "Could not create your account. Please try again or contact support.",
  // Verification / access
  Verification: "The sign-in link has expired. Please request a new one.",
  AccessDenied: "Access denied. If you believe this is an error, contact support.",
  // Server-side misconfiguration — env vars missing, DB down, etc.
  Configuration: "Server configuration error. Our team has been notified — please try again in a few minutes.",
...
function SignInForm() {
  ...
  const rawErrorCode = formError ? null : errorParam;
      ...
      <div className="relative z-10 w-full max-w-md animate-[fadeUp_0.7s_ease-out]">
        ...
        <Card className="border-border/50 shadow-2xl shadow-black/20 transition-all duration-300 hover:shadow-amber-500/10">
          ...
          <CardContent className="space-y-5">
            {error && <div className="rounded-lg border border-destructive/50 bg-destructive/10 px-4 py-3 text-sm text-destructive animate-[shake_0.4s_ease-out]"><p>{error}</p>{rawErrorCode && <p className="mt-1 text-[11px] opacity-70">Error code: <code className="font-mono">{rawErrorCode}</code> — share this with support if the issue persists.</p>}</div>}
